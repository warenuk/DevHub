        ...
        beforeOpen: (details) async {
          if (details.hadUpgrade && details.versionNow >= 3) {
            ...
            await customStatement(
              "UPDATE repos   SET fetched_at = COALESCE(fetched_at, CAST((strftime('%s','now') * 1000) AS INTEGER))",
            );
            await customStatement(
              "UPDATE commits SET fetched_at = COALESCE(fetched_at, CAST((strftime('%s','now') * 1000) AS INTEGER))",
            );
            await customStatement(
              "UPDATE activity SET fetched_at = COALESCE(fetched_at, CAST((strftime('%s','now') * 1000) AS INTEGER))",
            );
          }
        },
      );
