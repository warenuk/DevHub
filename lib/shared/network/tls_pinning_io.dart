...
void applyTlsPinningIfEnabled(Dio dio, Uri baseUri) {
  ...
  final host = (Env.tlsPinHost.isNotEmpty ? Env.tlsPinHost : baseUri.host).toLowerCase();
  final pin1 = Env.tlsPinCertSha256;
  final pin2 = Env.tlsPinCertSha256Backup;
  if (host.isEmpty || (pin1.isEmpty && pin2.isEmpty)) {
    AppLogger.error('TLS pinning enabled, but no host/pins provided. Skipping.', area: 'tls');
    return;
  }
  final adapter = IOHttpClientAdapter(
    createHttpClient: () {
      final client = HttpClient();
      client.badCertificateCallback = (X509Certificate cert, String certHost, int port) {
    ...
    try {
      ...
      final pem = cert.pem;
      final normalized = pem.replaceAll(RegExp(r'-----BEGIN CERTIFICATE-----|-----END CERTIFICATE-----|\s+'), '');
      ...
      if (!ok) {
        AppLogger.error('TLS pin mismatch for $certHost: got=${b64.substring(0,8)}..., expected one of pins', area: 'tls');
      } else {
        AppLogger.info('TLS pin OK for $certHost', area: 'tls');
      }
      return ok;
    } catch (e, s) {
      AppLogger.error('TLS pin check failed', error: e, stackTrace: s, area: 'tls');
      return false;
    }
      };
      return client;
